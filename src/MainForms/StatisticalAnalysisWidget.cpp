...
void StatisticalAnalysisWidget::on_tableWidget_itemChanged(QTableWidgetItem *item)
{
    ...
    if(item1 != 0)
    {
        if(item1->text().contains(","))
        {
            ...
            x1 = item1->text().left(index).toDouble();
            x3 = item1->text().right(item1->text().size()-index-1).toDouble();
        }
        else
        {
            x1 = item1->text().toDouble();
        }
    }
    if(item2 != 0)
    {
        if(item2->text().contains(","))
        {
            ...
            x2 = item2->text().left(index).toDouble();
            x4 = item2->text().right(item2->text().size()-index-1).toDouble();
        }
        else
        {
            x2 = item2->text().toDouble();
        }
    }
